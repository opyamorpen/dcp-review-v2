      ...
      <div style={S.content}>
        ...
        {!editing && (
          <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'flex-end' }}>
            <button style={S.btn(true)} onClick={() => { setEditing(true); setMessage('') }}>⚙️ 配置</button>
          </div>
        )}
...
const ChecklistConfigPanel: React.FC<{ items: any[]; roles: any[]; onChange: (v: any[]) => void; editing: boolean; phases: any[] }> = ({ items, roles, onChange, editing, phases }) => {
  ...
  return (
    <div>
      ...
      {editing && totalInPhase >= 20 && (
        <div style={{ marginTop: 16, color: '#faad14', fontSize: 12 }}>⚠️ 该阶段已达 20 项上限</div>
      )}
    </div>
  )
}
...
const NotifySettings: React.FC<{ config: any; onChange: (c: any) => void; editing: boolean }> = ({ config, onChange, editing }) => {
  ...
  return (
    <div>
      ...
      {!editing && <div style={{ marginBottom: 16, color: '#999', fontSize: 12 }}>点击右上角「⚙️ 配置」进入编辑模式后可修改</div>}
...
const ALL_CONCLUSIONS = [
  { value: 'pass', label: '✅ 通过' },
  { value: 'conditional_pass', label: '⚠️ 有条件通过' },
